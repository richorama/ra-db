namespace RaDb.Index
{
    ...
    public class BTree<TK, TP> : IBTree<TK, TP> where TK : IComparable<TK>
    {
        ...
        public Entry<TK, TP> SearchNearest(TK key)
        {
            return this.SearchNearestInternal(this.Root, key);
        }
        ...
        private Entry<TK, TP> SearchNearestInternal(Node<TK, TP> node, TK key)
        {
            int i = node.Entries.TakeWhile(entry => key.CompareTo(entry.Key) > 0).Count();
            ...
            if (!node.IsLeaf)
            {
                return this.SearchNearestInternal(node.Children[i], key);
            }
